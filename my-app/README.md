...
3. **Configure Auth0**
   ...
   **For different environments**: Update the `.env` file with environment-specific Auth0 credentials and configure your Auth0 Application settings with the appropriate URLs:
...
This app uses Auth0 for authentication. The configuration is stored in environment variables:

- **Domain**: Set via `VITE_AUTH0_DOMAIN` in `.env`
- **Client ID**: Set via `VITE_AUTH0_CLIENT_ID` in `.env`
- **Application Type**: Single Page Application (SPA)

**Note**: The `.env` file is not committed to version control. Copy `.env.example` to `.env` and add your Auth0 credentials. For SPAs, the Client ID is considered public information. Security is enforced through Auth0's application configuration (allowed URLs, etc.).
...
- AWS Amplify: Use environment variables in app settings
